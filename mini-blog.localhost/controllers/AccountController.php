...
{
    protected $auth_actions = array('index', 'signout', 'follow','edit');
    ...
    public function registerAction()
    {
        ...
        $name = $this->request->getPost('name');//追加
        ...
        if(mb_strlen($name)>=10){//追加
          $errors[]='名前は十文字以内にしてください';
        }

        if (count($errors) === 0) {
            $this->db_manager->get('User')->insert($user_name, $name,$password);//変更
    ...
    public function followAction($params)
    {
        ...
        if ($user['id'] !== $follow_user['id']
         ...
         }elseif($user['id'] !== $follow_user['id'] && $params['value']==='off'
       && $following_repository->isFollowing($user['id'], $follow_user['id'])){
    ...
    public function editAction(){//追加 //後でＣＳＦＲ
      $user = $this->session->get('user');

      if($this->request->isPost()){

        $token = $this->request->getPost('_token');
        if (!$this->checkCsrfToken('account/edit', $token)) {
            return $this->redirect('/account/edit');
        }

        $user['name']=$this->request->getPost('name');
        $user['intro']=$this->request->getPost('intro');
        $this->session->set('user',$user);

        $this->db_manager->get('User')->update($user['name'],$user['intro'],$user['id']);
        return $this->redirect('/account/edit');
      }else{

    return $this->render(array(
      'user' => $user,
      '_token'    => $this->generateCsrfToken('account/edit'),
    ));
    }
  }
